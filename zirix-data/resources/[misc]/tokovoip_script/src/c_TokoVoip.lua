...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if (string.match(self.myChannels[self.plugin_data.radioChannel].name, "Frequência")) then
			info = info  .. "   [ " .. self.myChannels[self.plugin_data.radioChannel].name;
		else
			info = info  .. "   [ Rádio: " .. self.myChannels[self.plugin_data.radioChannel].name;
		end
	end

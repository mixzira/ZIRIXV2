...
-----------------------------------------------------------------------------------------------------------------------------------------
src = {}
Tunnel.bindInterface("vrp_postman",src)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local amount = {}
...
function src.startPayments()
	...
	if user_id then
		...
		if data then
			if vRP.tryGetInventoryItem(user_id,"encomenda",amount[source]) then
				local price = math.random(100,140)
				local payment = price+(price*data.creative.carteiro*0.0001)
				...
				TriggerClientEvent("Notify",source,"aviso","Você precisa de <b>"..amount[source].."x Encomendas</b>.",8000)
			end
		end
		return false
	end

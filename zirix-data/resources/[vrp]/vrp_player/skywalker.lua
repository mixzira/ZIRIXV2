...
function src.checkPassPlatinum()
	local source = source
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"platinum.permissao") or vRP.hasPermission(user_id,"ultimate.permissao") then
		return true
	end
end
...
RegisterCommand('servico',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"policia.permissao") then
		TriggerClientEvent("Notify",source,"importante","<b>[DPLA]</b> Em serviço.")

	elseif vRP.hasPermission(user_id,"paisana-policia.permissao") then
		TriggerClientEvent("Notify",source,"importante","<b>[DPLA]</b> Fora de serviço.")

	elseif vRP.hasPermission(user_id,"ems.permissao") then
		TriggerClientEvent("Notify",source,"importante","<b>[DMLA]</b> Em serviço.")

	elseif vRP.hasPermission(user_id,"paisana-ems.permissao") then
		TriggerClientEvent("Notify",source,"importante","<b>[DMLA]</b> Fora de serviço.")

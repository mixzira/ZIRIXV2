...
function src.sellDealer(name)
	...
	if user_id then
		...
		local rows2 = vRP.query("losanjos/get_estoque",{ vehicle = name })
		if vehicle[1] then
			vRP.execute("losanjos/rem_vehicle",{ user_id = parseInt(user_id), vehicle = name })
			vRP.execute("losanjos/rem_srv_data",{ dkey = "custom:u"..parseInt(user_id).."veh_"..name })
			vRP.execute("losanjos/rem_srv_data",{ dkey = "chest:u"..parseInt(user_id).."veh_"..name })
			vRP.execute("losanjos/set_estoque",{ vehicle = name, quantidade = parseInt(rows2[1].quantidade) + 1 })
